<x-app-layout>
    <div class="max-w-6xl">
        <h1 class="mb-8 text-3xl font-bold text-slate-900">
            Preview Excel Import (Service Reports)
        </h1>
        ...
        @php
        $payload = session('correctives_import_preview');
        $rows = $payload['previewRows'] ?? [];
        $summary = $payload['summary'] ?? [];
        $errors = $payload['errors'] ?? [];

        $debugCounts = session('correctives_import_preview_debug');
        @endphp
            ...
            <table class="min-w-full">
                <thead class="bg-slate-50">
                    <tr class="border-t transition hover:bg-slate-50">
                        <th class="px-6 py-4 text-left">Tanggal Laporan</th>
                        <th class="px-6 py-4 text-left">Jam Laporan</th>
                        <th class="px-6 py-4 text-left">Jam Visit</th>
                        <th class="px-6 py-4 text-left">Time Response</th>
                        ...
                        <th class="px-6 py-4 text-left">Problem</th>
                ...
                <tbody>
                    @forelse($rows as $row)
                    <tr class="border-t transition hover:bg-slate-50">
                        <td class="px-6 py-4 text-sm text-slate-700">{{ $row['repair_date'] ?? '—' }}</td>
                        <td class="px-6 py-4 text-sm text-slate-700">{{ $row['jam_laporan'] ?? '—' }}</td>
                        <td class="px-6 py-4 text-sm text-slate-700">{{ $row['jam_visit'] ?? '—' }}</td>
                        <td class="px-6 py-4 text-sm text-slate-700">{{ $row['response_time'] ?? '—' }}</td>
                        <td class="px-6 py-4 text-sm text-slate-700">{{ $row['room'] ?? '—' }}</td>
                        <td class="px-6 py-4 text-sm text-slate-700 font-medium">{{ $row['asset_name'] ?? '—' }}</td>
                        ...
                        <td class="px-6 py-4 text-sm text-slate-700">{{ $row['problem'] ?? '—' }}</td>
                        <td class="px-6 py-4 text-sm text-slate-700">{{ $row['solution'] ?? '—' }}</td>
                        <td class="px-6 py-4 text-sm text-slate-700">{{ $row['sparepart'] ?? '—' }}</td>
                        <td class="px-6 py-4 text-sm text-slate-700">{{ $row['quantity'] ?? '—' }}</td>
                        <td class="px-6 py-4 text-sm text-slate-700">{{ $row['inspection_result'] ?? '—' }}</td>
                        <td class="px-6 py-4 text-sm text-slate-700">{{ $row['user_name'] ?? '—' }}</td>
                    </tr>
                    @empty
                    <tr>
                        <td colspan="15" class="px-6 py-8 text-center text-sm text-slate-500">
                            No rows to preview.
                        </td>
                    </tr>
                    @endforelse
